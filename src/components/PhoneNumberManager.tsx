...
export default function PhoneNumberManager({ lang }: PhoneNumberManagerProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-5 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 space-y-4">
        ...
        {prefs?.verified ? (
          <div className="flex items-center justify-between p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
            <div>
              <p className="font-mono text-lg font-bold text-emerald-600 dark:text-emerald-400 tracking-wider">
                {prefs.phoneNumber}
              </p>
            ...
            {!mockCodeSent ? (
              <>
                <div className="flex items-center gap-2 w-full max-w-full">
                  <div className="relative shrink-0">
                    <select 
                      ...
                      className={`h-12 w-[100px] bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-3 text-slate-800 dark:text-slate-200 text-sm font-mono appearance-none focus:outline-none focus:border-indigo-500 ${isRtl ? 'pl-8' : 'pr-8'}`}
                    >
                      {COUNTRIES.map(c => (
                        <option key={c.code} value={c.code}>{c.code} ({isRtl ? c.nameAr : c.nameEn})</option>
                      ))}
                    </select>
                    <ChevronDown className={`w-4 h-4 text-slate-700 dark:text-slate-400 absolute top-4 pointer-events-none ${isRtl ? 'left-3' : 'right-3'}`} />
                  </div>
                  <input
                    type="tel"
                    ...
                    className="flex-1 min-w-0 w-full h-12 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 text-slate-800 dark:text-slate-200 font-mono tracking-wider focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20"
                >
                  {isVerifying ? (
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  placeholder="000000"
                  className="w-full h-14 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl px-4 text-center text-2xl tracking-[0.5em] font-mono text-slate-800 dark:text-slate-200 focus:outline-none focus:border-indigo-500 transition-colors"
                />
                {error && <p className="text-[11px] text-rose-600 dark:text-rose-400 font-medium flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {error}</p>}
                <button
                  onClick={handleVerifyCode}
                  disabled={isVerifying || verificationCode.length < 6}
                  className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-slate-900 dark:text-white font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
                >
                  {isVerifying ? (
                    <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                  ) : (
      ...
      <div className={`rounded-2xl p-5 bg-white dark:bg-[#0f172a] border ${prefs?.verified ? 'border-indigo-500/30' : 'border-slate-200 dark:border-slate-800 opacity-60 pointer-events-none'} transition-all`}>
        <div className="flex items-center gap-3 mb-6">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${prefs?.verified ? 'bg-indigo-500/10 border-indigo-500/20' : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700'}`}>
            <MessageSquare className={`w-5 h-5 ${prefs?.verified ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-700 dark:text-slate-400'}`} />
          </div>
          <div>
      ...
      <div className="rounded-2xl p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3">
...
function ToggleRow({ label, desc, isActive, onToggle, isDisabled = false, isMaster = false }: {
  ...
  return (
    <div className={`flex items-center justify-between gap-4 ${isDisabled ? 'opacity-50 pointer-events-none' : ''}`}>
      <div>
        <p className={`text-sm font-semibold ${isMaster ? 'text-indigo-500 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-300'}`}>{label}</p>
        <p className="text-[11px] text-slate-700 dark:text-slate-400 mt-0.5">{desc}</p>
      </div>
      <button
        onClick={onToggle}
