...
export default function Avatar({ uid, className = "w-7 h-7", iconClassName = "w-4 h-4" }: AvatarProps) {
  ...
  return (
    <div className={`rounded-full overflow-hidden bg-bg-secondary flex items-center justify-center shrink-0 border border-slate-300 dark:border-slate-700 cursor-pointer ${className}`}>
      {photoURL ? (
        <img src={photoURL} alt="User Avatar" className="w-full h-full object-cover" />
      ) : (
        <UserIcon className={`text-text-primary dark:text-text-secondary ${iconClassName}`} />
      )}
    </div>
  );
}
